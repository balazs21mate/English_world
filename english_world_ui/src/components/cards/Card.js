...
function Card({title, list}){
    ...
    return(
        <div className='flex flex-col justify-center items-center h-[20rem] max-w-lg w-[90%]'>
            ...
            <small></small>
            <div className={`${rotate? 'rotate_card':'rotate_card_origin'} ${mirrorText&&'rotate_card_null'} ${any&&'rotate_card_180'} shadow-card flex justify-center items-center text-secondary_color w-full h-full border-solid border border-black rounded-lg mb-8`} onClick={buttonDisabled?null:set_rotate}>{text}</div>
